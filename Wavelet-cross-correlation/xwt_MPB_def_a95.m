function [coi,signif,Pkx,Pky,Wxy,t,period, sig95,sigmax,sigmay,X,Y]=xwt_MPB_def_a95(x,y,a,varargin)
...
%
% -------------------------------------------------------------------------
% Modified by M. Puente-Borque (2025) from (2014 Aslak Grinsted)
%http://www.glaciology.net/wavelet-coherence

%---------------------------------------------------------------------------
addpath('wavelet-coherence-master\')
addpath('wavelet-coherence-master\private22\')
%a=0.999
% ------validate and reformat timeseries.
[x,dt]=formatts(x);% x señal y dt frecuencia temporal de muestro
[y,dty]=formatts(y);% deben ser igualmente espaciadas ambas señales
if dt~=dty
...
%-----------:::::::::::::--------- ANALYZE ----------::::::::::::------------
inc = abs(x(1,1)-x(2,1));
Fs = 1/inc;  %frecuencia de muestreo
% Cálculo de Wavelets: X=Wx e Y=Wy. coi=cone of influence
[X,period,coix] = cwt_program(x(:,2),'amor',years(1/Fs),'VoicePerOctave',48);%#ok
[Y,period,coiy] = cwt_program(y(:,2),'amor',years(1/Fs),'VoicePerOctave',48);
period=years(period'); coix=years(coix'); coiy=years(coiy');
...
% -------- Cross ------------------------------
%X=X/max(max(abs(X))); Prueba de Marina para calibrar la sensibilidad 
%Y=Y/max(max(abs(Y)));
Wxy=X.*conj(Y); %cross correlation

%--------- Sigma  95---------------------------- 
% Pkx=ar1spectrum(Args.AR1(1),period./dt);
% Pky=ar1spectrum(Args.AR1(2),period./dty);
Pkx=ar1spectrum(a,period./dt);
Pky=ar1spectrum(a,period./dty);

V=2; % V=1 si Wavelet es real / V=2 si wavelet compleja(morlet y amor son C) 
Zv=3.999; % Zv(p). En este caso Z2(95%) Nivel de confianza asociado con 
%            la probabilidad p, Z2(95%)=3.999 ver (Torrence and Compo 1998)
    


signif=sigmax*sigmay*sqrt(Pkx.*Pky)*Zv/V;
sig95 = (signif')*(ones(1,n));  % expand signif --> (J+1)x(N) array
sig95 = abs(Wxy) ./ sig95;%ver(Grinsted et al 2004;Torrence and Compo 1998)
...
% freq = dt ./ period;
    ...
    H=imagesc(t,log2(period),(abs(Wxy/(sigmax*sigmay))));%#ok
...
%     phaseplot(t(tidx),log2(period(pidx)),aWxy(pidx,tidx),Args.ArrowSize,Args.ArrowHeadSize);
    ...
    %hold on
%     [c,h] = contour(t,log2(period),abs(sig95),[1 1],'Color','k');%#ok
%     set(h,'linewidth',2)
%      
    hold on
    tt=[t([1 1])-dt*.5;t;t([end end])+dt*.5];
    hcoi=fill(tt,log2([period([end 1]) coi period([1 end])]),'w');
    set(hcoi,'alphadatamapping','direct','facealpha',.1)
    ...
    pot = minY:1:maxY;
    set(gca,'Color','black')
    set(gca,'YTick',pot,'YTickLabels',2.^pot)
    set(gcf,'Color','white')
    set(gcf, 'InvertHardcopy', 'off')
    axis ij
    colormap("turbo")
%     hold on
%     yline(log2(1300),'w--','linewidth',2)
    set(gca,'FontSize',14)
%
